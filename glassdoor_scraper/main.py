...
def scrape(field, review, author):
    symbol_ratings = {'css-1h93d4v': 'no opinion',
                    'css-hcqxoa' : 'positive',
                    'css-10xv9lv': np.nan,
                    'css-1kiw93k': 'negative'
                    }

    def remove_suffix(input_string):
        suffix = ' people found this review helpful\nHelpful\nShare\nRepor'
        suffix_b = '\nBe the first to find this review helpful\nHelpful\nShare\nRepor'
        if input_string.endswith(suffix):
            example = input_string[:-len(suffix)]
            cleaned = re.sub(r'\d+$', '', example)
            return cleaned.split('\nAdvice to Management\n')[0].strip()
        elif input_string.endswith(suffix_b):
            return input_string[:-len(suffix)].split('\nAdvice to Management\n')[0].strip()
        return input_string

    def scrape_date(author):
        
        date = author.find_element_by_class_name(
                    'authorJobTitle').text.split('-')[0]#review.find_element_by_tag_name(
            #'time').get_attribute('datetime')
        #time_index = date.find(':') - 3
        res = dt.datetime.strptime(date.strip(), '%b %d, %Y')#[:time_index]
        return res
    ...
    def scrape_status(review):
        try:
            res = review.find_element_by_class_name('eg4psks0').text#author.text.split('-')[1]
        except Exception:
    ...
    def scrape_rev_title(review):
        return review.find_element_by_class_name('el6ke055').text.strip('"')

    def scrape_helpful(review):
        try:
            helpful = review.find_element_by_class_name('common__EiReviewDetailsStyle__socialHelpfulcontainer')
            count = helpful.text.split('people')
            res = 0
            if len(count)>1:
                res = count[0]
        except Exception:
    ...
    def scrape_cons(review):
        try:
            ...
            res = remove_suffix(cons.text[con_index+5 : continue_index])
        except Exception:
            res = np.nan
        return res
    ...
    def scrape_advice(review):
        try:
            ...
            if advice_index != -1:
                ...
                res = remove_suffix(advice.text[advice_index+21 : helpful_index])
            else:
                res = np.nan
        except Exception:
            res = np.nan
    ...
    def _scrape_subrating(i):
        ...
        try:
            #logger.info(rating_scale)
            ratings = review.find_element_by_class_name('gd-ui-tooltip-info')
            subratings = ratings.find_element_by_class_name(
                'content').find_element_by_tag_name('ul')
            this_one = subratings.find_elements_by_tag_name('li')[i]
            
            css_rating = this_one.find_element_by_class_name(
                'e1hd5jg10').get_attribute('class').split(' ')[0]
            res = rating_scale[css_rating]
        except Exception:
            logger.info("Failed at getting subrating.")
            res = np.nan
        return res
    ...
    def scrape_recommends(review):
        try:
            categories = review.find_element_by_class_name('recommends')
            recommends = categories.find_elements_by_class_name('mr-std')[0]
            span = recommends.find_element_by_class_name('d-flex').get_attribute('class').split(' ')[1]
            #logger.info(span)
            res = symbol_ratings[span]
            return res
        except:
            return np.nan
    ...
    def scrape_outlook(review):
        try:
            categories = review.find_element_by_class_name('recommends')
            recommends = categories.find_elements_by_class_name('mr-std')[2]
            span = recommends.find_element_by_class_name('d-flex').get_attribute('class').split(' ')[1]
            #logger.info(span)
            res = symbol_ratings[span]
            return res
    ...
    def scrape_approve_ceo(review):
        try:
            categories = review.find_element_by_class_name('recommends')
            recommends = categories.find_elements_by_class_name('mr-std')[1]
            span = recommends.find_element_by_class_name('d-flex').get_attribute('class').split(' ')[1]
            #logger.info(span)
            res = symbol_ratings[span]
            return res
...
def navigate_to_reviews():
    ...
    reviews_cell = browser.find_element_by_xpath(#'//*[@id="ReviewsRef"]')
...
def sign_in():
    ...
    # import pdb;pdb.set_trace()
    cookies_btn = browser.find_element_by_id("onetrust-accept-btn-handler")
    cookies_btn.click()
